...
session request budget. The no-profile and competition-nebius entries retain
checked generation/review. Switching profiles does not reset memory consent.

The opt-in Agent interaction control is session-only and defaults off. Its
selective narrow check also defaults off, uses at most one extra request and
cannot expand permissions. An unknown or failed required check discards the draft
before history and memory commit. Detection is bounded and incomplete; successful
fixtures do not prove semantic reliability, naturalness or clinical benefit.
